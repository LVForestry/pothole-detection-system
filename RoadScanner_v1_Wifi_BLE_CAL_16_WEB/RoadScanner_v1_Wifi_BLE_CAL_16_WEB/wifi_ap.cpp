...
<body>
  ...
  <script>
      ...
      fetch('/sensors')
        ...
        .then(data => {
          ...
          data.forEach(sensor => {
            ...
            // Ajouter un bouton "Calibrer" pour chaque capteur
            const button = document.createElement('button');
            button.textContent = 'Calibrer';
            button.onclick = () => calibrateSensor(sensor.name);
    ...
    function calibrateSensor(sensorName) {
      ...
      fetch(`/calibrate?sensor=${sensorName}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Erreur lors de la calibration');
          }
          return response.json();
        })
        .then(data => {
          if (data.ok) {
            alert(`Calibration réussie pour le capteur : ${sensorName}`);
          } else {
            alert(`Erreur de calibration : ${data.error}`);
          }
        })
        .catch(error => {
...
void wifi_ap_handleClients() {
  ...
  if (reqLine.startsWith("GET /sensors")) {
    ...
    // Extraire le nom du capteur depuis la requête
    int sensorStart = reqLine.indexOf("sensor=") + 7;
    int sensorEnd = reqLine.indexOf(' ', sensorStart);
    String sensorName = reqLine.substring(sensorStart, sensorEnd);
    sensorName.trim();
    Serial.print("Calibration demandée pour le capteur : ");
    Serial.println(sensorName);

    // Appeler la fonction de calibration (à implémenter dans sensor_manager)
    String response = sensor_manager_calibConnect(sensorName);
    client.print("HTTP/1.1 200 OK\r\n");
    client.print("Content-Type: application/json\r\n");
    client.print("Connection: close\r\n");
